 ...
 * ```
 *
 * With `NETWORKS` unset the flat `HORIZON_URL` / `NETWORK_PASSPHRASE`
 * variables become a single network named by `PRIMARY_NETWORK` (default
 * `mainnet`), so an existing single-network deployment keeps working without a
 ...
 * string.
 *
 * This file is deliberately a second copy of `indexer/src/networks.ts` rather
 * than a shared import: the two packages don't share code (see
 * `graphql-server/src/horizon.ts` and `notifications.ts` for the same rule),
 * and each service reads its own configuration at its own startup. The
 * parsing rules are kept identical — a deployment must not resolve a
 * different primary network in the indexer than it does in the API.
 */
...
/** The registry for this process, resolved once at first use. */
let cached: NetworkRegistry | null = null;

/**
 * The deployment's configured networks.
 *
 * Resolved lazily so a config error surfaces as a startup failure (the server
 * calls this before it listens) rather than at import time, and so tests can
 * call `resolveNetworks` with their own environment instead of this cache.
 */
export function getNetworks(): NetworkRegistry {
  if (cached === null) cached = resolveNetworks(process.env);
  return cached;
}

/** The value the `Network` enum carries for a configured network name. */
export function networkEnumValue(name: string): string {
  return name.toUpperCase();
}

/**
 * Turn the optional `network:` argument of a query into the network to serve.
 *
 * Omitted means the configured primary — which is what every client saw before
 * the argument existed, and why adding it is not a breaking change. Named
 * means that network and no other: a network this deployment does not
 * configure is an error rather than a silent fall back to the primary,
 * because "I asked for testnet and got mainnet" is exactly the data mixing
 * this argument exists to prevent.
 */
export function resolveNetworkArgument(
  requested: string | null | undefined,
  registry: NetworkRegistry = getNetworks()
): NetworkConfig {
  if (requested === undefined || requested === null) return registry.primary;

  const name = requested.toLowerCase();
  const match = registry.networks.find(network => network.name === name);
  if (!match) {
    throw new GraphQLError(
      `Network "${requested}" is not configured on this deployment. ` +
        `Configured networks: ${registry.networks.map(network => networkEnumValue(network.name)).join(', ')}.`,
